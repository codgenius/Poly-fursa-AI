...
AGENT_INPUT_TOKENS = Counter(
    ...
    "Total number of input tokens processed by the agent.",
)
AGENT_OUTPUT_TOKENS = Counter(
    "agent_output_tokens_total",
    "Total number of output tokens generated by the agent.",
)
...
def run_agent(history: list, max_iterations: int = 10) -> ChatResponse:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        AGENT_INPUT_TOKENS.inc(usage.get("input_tokens", 0))
        AGENT_OUTPUT_TOKENS.inc(usage.get("output_tokens", 0))
...
async def observe_chat_requests(request, call_next):
    """Record assignment-specific metrics for the chat endpoint only."""
    ...
    try:
        ...
        AGENT_CHAT_REQUESTS.labels(status=status).inc()
        AGENT_CHAT_REQUEST_DURATION.observe(time.perf_counter() - started_at)
